...
**Known risks carried forward.** `ProceduralQuestTest` and the Drama projector write to saves and
are gated behind config flags; those flags are the current safety boundary and must not be removed
until §S9. Three Harmony patches now exist in `DramaChoiceProjector` — that is the project's main
version-drift exposure.
