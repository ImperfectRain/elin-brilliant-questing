...
**Working headless.** 142 tests. Deterministic simulation core: stable identity, event ledger,
...
**Known gaps at the code level.** Vanilla theft is observed through `EVENT.ActPerformed`, but
observed witnesses are not yet derived from proximity or line of sight. No Home state. No
