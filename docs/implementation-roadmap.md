...
```
BQ-014 Observe vanilla theft through ActPerformed
```
...
Verified against the tree and against `BepInEx/LogOutput.log` from a live game on 28 Aug 2026.
...
**Working headless.** 226 tests. Deterministic simulation core: stable identity, event ledger,
fact/belief separation with provenance and provability, memory with consolidation, relationship
graph, threads with milestone escalation, fifteen verbs across six solution families, four-outcome
...
**Known gaps at the code level.** Vanilla theft and combat/death results are observed through
`EVENT.ActPerformed`, with witnesses derived from proximity, line of sight and stealth/perception,
then routed through the consequence engine. Provable beliefs now carry proof links to physical
evidence or testimony, and an auditor checks those links against the world. Authority figures have
explicit report outcomes for physical proof, witness-backed proof, unprovable belief and rumor.
Rumours circulate through a live town on a bounded daily schedule, fading when nobody repeats them
and never carrying proof; they can garble in the retelling and be told wrong on purpose, producing
false beliefs that stand beside the truth in the graph rather than replacing it, and that people act
on. Memory compaction has a long-save budget: old trivia decays, routine patterns consolidate, and
Defining memories are never deleted. Harm travels along the relationship graph, so hurting somebody
costs whatever the people tied to them think of it, damped by how close the tie is and how warm it
is, and derived from the edge rather than from any rule naming the pair. Debt can be settled by real
orens without a social check. No Home state. No broader economy. No sites. No NPC autonomy. No
director. Two situation archetypes. Personality exists as nine decision weights and is not yet
expressed in dialogue. The scenario is staged by a debug flag, not generated by the world.

**Known risks carried forward.** `StageScenarioOnLoad` and `GatherPrototypeNpcsNearPlayer` are the
two flags that write to or move things in a save; both default to off and must stay opt-in until
...
| Action library | Playable (15 verbs) | Complete-until-launch (~40) | BQ-023 … BQ-029 |
| Threads & escalation | Playable | Hardened | BQ-013, BQ-052 |
| Situation archetypes | Prototype (2) | Complete-until-launch (7) | BQ-041 … BQ-047 |
