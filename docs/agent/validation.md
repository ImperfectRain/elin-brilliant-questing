...
| Character/goals/emotion | `ProblemSolvingStyleTests`, `NpcGoalContractTests`, `ActorGoalEvolutionTests`, `EmotionalStateTests`, affected profile tests | `ActorLocalInterpretationTests`, `DisclosureDecisionTests`, `MigrationFixtureTests`, `ActorActionScenarioTests` (Lab project); `actor-action`, `playground` |
...
| Competition over one indivisible thing, claims and batch arbitration | `ActionArbitrationTests` | `ActionOpportunityTests`, `ActionEffectContractTests`, `PlayerNpcActionSymmetryTests`, `FoundationTests`, persistence; `theft`, `actor-action` |
...
| Selected Core fulfillment and atomic publication | `SituationEstablishmentTests` | `SituationProposalEcologyTests`, `SituationProposalTests`, `SettlementSituationGeneratorTests`, `CausalProvenanceTests`, `ConsequenceTests`, persistence gates; `integration`. Inject failure before/after preparation and each staged write, verify post-commit listener errors/reentry, stale bindings/budgets, monotonic IDs and retry/save stability |
| Cross-matter shared causes and independent resolution | `CrossMatterCompositionTests` | `SituationEstablishmentTests`, `SituationProposalEcologyTests`, `PressureSynthesisTests`, `ConsequenceTests`, `ProductionActionTests`, `ThreadLifecycleTests`, persistence gates; `integration`. Compare one versus several matters for single-dispatch effects; preserve separate condition carriers, histories and later establishment after resolution |
...
| Capability disable drill | `CapabilityDegradationTests`, adapter route above and `VanillaActionRecorderTests` | [One-capability-at-a-time drill](../elin/verification/runtime-probes.md#drill-implementation-and-procedure); headless probe isolation is not live feature isolation |
| Elin update smoke | Plugin build against updated game libraries; docs-only checklist edits use link validation | [Update smoke checklist](../elin/verification/update-smoke-test.md): identified real update, aliases, patch execution, existing chunk, played situation and reload |
| Runtime evidence instrumentation | `RuntimeEvidenceTests`, `ElinHomeStateTests`, `VanillaActionRecorderTests`; Plugin build | [Performance and Home capture](../elin/verification/runtime-probes.md#performance-and-home-capture); actual measurements remain unverified until returned |
