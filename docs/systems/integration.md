...
| Selected establishment record (additive optional thread node, no bump) | Preserve its stable ID, producer/cause identity and committed role bindings. Missing/null means no selected establishment; never infer one for an old matter. Malformed records, mismatched origin occurrences and duplicate identities quarantine the affected thread. Requirement keys, selection tokens, staging and notification diagnostics are transient; historical occurrences are never redispatched. See [generation](world.md#generation) and `SituationEstablishmentTests` |
