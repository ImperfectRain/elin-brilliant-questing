...
Current live HEAD is `BQ-037`. Important next dependencies:

- `BQ-037/BQ-038` guild networks and authority: the player's own membership works; rank and per-guild contribution are read from `FactionRelation.rank/exp` since `BQ-038` (`SOURCE-OBSERVED`; not read on a running member save, so the rank scale the authority threshold uses is unverified). Which of the four guilds a *live NPC* belongs to has no identified read, so BQ-037's routing has no members and BQ-038 has no officer who speaks for a guild in a real game until a generator or an adapter grants the membership role (`ELIN-Q-0025`, `UNRESOLVED`).
