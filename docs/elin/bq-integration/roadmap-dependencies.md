...
- `BQ-037/BQ-038` guild networks: the player's own membership works; numeric rank/progression exists on `FactionRelation.rank/exp/ExpToNext`, but current BQ returns binary rank (`SOURCE-OBSERVED`, `FIX-ELIN-007`). Which of the four guilds a *live NPC* belongs to has no identified read, so BQ-037's routing has no members in a real game until a generator or an adapter grants the membership role (`ELIN-Q-0025`, `UNRESOLVED`).
